...
#if defined(MIYOO)
    window = SDL_CreateWindow(NULL, window_x, window_y, 640, 480, window_flag);
#else
...
#if defined(MIYOO)
    SDL_RenderSetLogicalSize(renderer, 640, 480); //Auto scale center
#else
...
void ONScripter::flushDirect( SDL_Rect &rect, int refresh_mode )
{
	...
	p.x = 640 / 2;//320;
	p.y = 480 / 2;//240;
	SDL_RenderCopyEx(renderer, texture, NULL, &rect_, 0/*270*/, &p, SDL_FLIP_NONE);
#endif
